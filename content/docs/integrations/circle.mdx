...
- **Action:** *Send webhook*
  ...
  - **Header:** `X-AnswerLoops-Token` = the secret from step 2 (use the header whenever the Workflow action supports it — if it can't set headers, append `&token=<secret>` to the URL instead)
